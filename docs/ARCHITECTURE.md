...
| **ClickHouse** *(Phase 5)* | 24.x | OLAP store for funnel + revenue analytics, fed by the `analytics` service consuming `order.*` / `payment.*` events into a wide `events` MergeTree + rollup materialized views. Grafana queries it directly (`grafana-clickhouse-datasource`). | Single-node in compose; ClickHouse Keeper + replicas in prod. Browser clickstream ingestion is a follow-on. |
...
| **analytics** | OLAP sink — flattens domain events into ClickHouse for funnel + revenue analytics. Consumer-only (no gRPC API). | — (health only) | — | `order.{created,confirmed,cancelled,fulfilled}`, `payment.{authorized,failed,refunded}` |
...
| **5 — Scale/optional** | ✅ ClickHouse analytics (`analytics` service → `events` + funnel MVs; Grafana ClickHouse datasource + funnel dashboard); browser clickstream ingestion; CDN; multi-zone; OpenFGA fine-grained authz; marketplace/multi-seller model. |
