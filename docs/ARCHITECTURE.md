...
| **MinIO** | latest | S3-compatible object storage: product images/video, generated invoices (PDF), data exports, archived security-scan reports. | `media` service is the only writer. Presigned URLs for client uploads. **Reads are served through a CDN** (`MEDIA_PUBLIC_BASE_URL`), not from MinIO directly — see §2.3. Versioning + lifecycle rules on. |
| **ClickHouse** *(Phase 5)* | 24.x | OLAP store for analytics, fed by the `analytics` service: `order.*` / `payment.*` domain events into a wide `events` MergeTree (funnel + revenue), and browser `clickstream.tracked` events into a `clickstream` MergeTree (page/product views, search, cart), each with rollup materialized views. Grafana queries it directly (`grafana-clickhouse-datasource`). | Single-node in compose; ClickHouse Keeper + replicas in prod. |
...
- **CDN** *(Phase 5)* — product-media reads are served through a caching edge, not from
  ...
  store as origin; compose runs an nginx `proxy_cache` stand-in (`deploy/compose/cdn/`,
  `GET`/`HEAD` only, `/product-media/` only, query-string-independent cache key, immutable
  `Cache-Control`, `X-Cache-Status`). Hit ratio / origin-offload land in the `cdn-cache`
  Grafana dashboard (nginx access log → Loki) plus request-rate metrics via
  `nginx-prometheus-exporter`. See [DECISIONS.md ADR-036](DECISIONS.md).
...
| **catalog** | Products, variants, categories, attributes, media references, **per-shop ownership** (`shop_id`; a seller's writes are authz'd via OpenFGA `product#manager`, `catalog_manager` still covers any listing) | `GetProduct`, `ListProducts`, `BatchGetProducts`, `CreateProduct`, `UpdateProduct`, `ArchiveProduct`, `ListShopProducts` | `catalog.product_changed`, `catalog.category_changed` | `media.asset_ready` |
...
| **payout** | Per-shop payouts — groups a confirmed order's lines by shop into one `Payout` per shop per order, **100% passthrough, no platform commission**; SANDBOX settlement sweep PENDING → PAID. An approved return **reverses** the matching shop's payout, cumulatively, clamped at what was owed. Reads gated by OpenFGA `shop#staff` or the `finance` role. [ADR-043](DECISIONS.md), [ADR-045](DECISIONS.md) | `ListPayouts`, `GetPayout`, `MarkPaid` | `payout.created`, `payout.paid`, `payout.reversed` | `order.confirmed`, `order.return_approved` |
...
- ✅ **KEDA `ScaledObject`** per autoscaled service (`templates/scaledobject.yaml`), one
  ...
  the app-of-apps.
- ✅ `PodDisruptionBudget` — `minAvailable: 1` by default; `bff` / `order` / `payment` use
  `maxUnavailable: 34%` so a one-zone-at-a-time node drain proceeds but a whole-service
  eviction is blocked.
...
- ✅ **Multi-zone topology** (`global.multiZone.enabled`, off for kind):
  `topologySpreadConstraints` on `kubernetes.io/hostname` (soft) **and**
  `topology.kubernetes.io/zone` — `ScheduleAnyway` by default, `DoNotSchedule` for
  `bff`/`order`/`payment` (`replicas ≥ 3`) to guarantee a per-zone copy; both keyed with
  `matchLabelKeys: [pod-template-hash]` so a rollout spreads on its own axis. Plus a
  per-service Istio **`DestinationRule`** (`<svc>-locality`): `localityLbSetting`
  (`failoverPriority: zone → region`) keeps east-west gRPC in-zone, `outlierDetection`
  ejects a degraded zone so failover fires, bounded `connectionPool`. A single-AZ loss is a
  non-event — [RUNBOOKS.md RB-9](RUNBOOKS.md). See [DECISIONS.md ADR-037](DECISIONS.md).
...
- **Capacity**: load tests (`k6`) replicating the checkout funnel — `perf/checkout-funnel.js`,
  ...
  test also gates checkout-path latency (p95 budgets, see [`perf/README.md`](../perf/README.md)).
...
| **5 — Scale/optional** | ✅ ClickHouse analytics (`analytics` service → `events` + funnel MVs; Grafana ClickHouse datasource + funnel dashboard); ✅ browser clickstream ingestion (storefront beacon → BFF `POST /api/v1/events` → `commerce.clickstream.tracked` → `analytics` second consumer group → `clickstream` table + dashboard); ✅ OpenFGA fine-grained authz (ReBAC store + `pkg/fga`; delegated order sharing — `OrderService.ShareOrder`/`RevokeOrderShare`/`ListOrderShares`, additive over role + owner checks); ✅ CDN for product media (served-URL host = caching edge; nginx `proxy_cache` stand-in in compose + `cdn-cache` dashboard; managed CDN in prod); ✅ multi-zone (zone-spread pods with `DoNotSchedule` for bff/order/payment, `maxUnavailable: 34%` PDBs, per-service Istio locality-LB `DestinationRule` with zone failover; RB-9); ✅ **marketplace/multi-seller model** — complete: ✅ `seller` service (shop aggregate + onboarding, `shop.*` events, `shop_admin` role); ✅ OpenFGA shop-staff (`shop#owner`/`shop#staff`, canonical model centralised in `pkg/fga`); ✅ per-shop catalog ownership (`products.shop_id` + `product#manager`, catalog enforces seller writes; BFF `/seller/products` CRUD + `GET` list via `ListShopProducts`); ✅ storefront "sold by" (public `ListProducts` gains a `shop_id` filter; BFF `GetProduct` attaches `sold_by` for marketplace products, degrading gracefully on lookup failure; new public `GET /api/v1/shops/:slug` + `.../products`; storefront PDP "Sold by" link + `/shops/:slug` shop page); ✅ fulfillment split by shop (`shop_id` threaded catalog → pricing → order lines; fulfillment groups a confirmed order's lines by shop into one shipment per shop, `shipments` unique on `(order_id, shop_id)`; the order only reaches FULFILLED once every shop group's shipment has delivered, tracked via `order_shipment_deliveries`); ✅ per-shop payouts (new `payout` service; consumes `order.confirmed`, groups+sums lines by shop into one `Payout` per shop per order, no platform commission yet; SANDBOX settlement sweep PENDING→PAID mirroring the fulfillment carrier; reads gated by OpenFGA `shop#staff` or the `finance` role; BFF `GET /seller/payouts` + admin `GET /admin/payouts` / `POST /admin/payouts/:id/mark-paid`); ✅ seller dashboard (`web/storefront` `/seller/*` routes — shop settings + onboarding, product CRUD, staff management, payout ledger — reusing the existing BFF surface, no backend changes). |
