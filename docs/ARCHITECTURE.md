...
| **fulfillment** | Shipments, carrier labels, tracking, returns/RMA | `CreateShipment`, `GetTracking`, `CreateReturn` | `fulfillment.shipped`, `fulfillment.delivered`, `fulfillment.return_received` | `order.confirmed` |
...
| **3 — Fulfillment & comms** | `fulfillment`, `notification`, `review`; admin dashboard + admin `HTTPRoute` with IP allow-list; RMA/returns. |
