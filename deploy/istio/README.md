...
| `authorization-policy.yaml` | **gateway-scoped only**: ext-authz `CUSTOM`, namespace `default-deny`, admin source-IP allow-list. The per-service `allow-to-<svc>` L7 policies (and `NetworkPolicy`) are generated by `deploy/helm/commerce-services`. |
